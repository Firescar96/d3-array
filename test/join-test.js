...
tape("join returns the cartesian product", function(test) {
  test.deepEqual(arrays.join([1,2,3], [4,5,6]).apply(), [[1,4],[1,5],[1,6],[2,4],[2,5],[2,6],[3,4],[3,5],[3,6]]);
  test.deepEqual(arrays.join([1,2], [4,5,6]).apply(), [[1,4],[1,5],[1,6],[2,4],[2,5],[2,6]]);
  test.deepEqual(arrays.join([1,2,3], [4,5]).apply(), [[1,4],[1,5],[2,4],[2,5],[3,4],[3,5]]);
  test.end();
});
...
tape("join can change the data", function(test) {
  let operation = arrays.join([{a:1,z:11},{a:2,z:12},{a:3,z:13}], [{a:4,b:7},{a:5,b:8},{a:6,b:9}]);
  operation.data([{a:1}], [{a:4},{a:5},{a:6},{a:7}]);
  test.deepEqual(operation.apply(), [[{a:1},{a:4},],[{a:1},{a:5}],[{a:1},{a:6}],[{a:1},{a:7}]]);
  test.end();
});
